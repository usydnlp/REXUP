...
class MACnet(object):
    ...
    def __init__(self, embeddingsInit, answerDict, questionDict, sceneDict, nextElement=None):
    ...
    def addPlaceholders(self):
        with tf.variable_scope("Placeholders"):
            ...
            if config.imageEnsembleFeatures:
                if "spatial" in config.imageEnsembleChoices:
                    self.imagesPlaceholder = tf.placeholder(tf.float32, shape=(None, None, None, None))
                    self.spatial_imagesAll = tf.transpose(self.imagesPlaceholder, (0, 2, 3, 1))
                if "object" in config.imageEnsembleChoices:
                    self.object_imagesAll = tf.placeholder(tf.float32, shape=(None, None, None))
                    self.object_imageBBoxesAll = tf.placeholder(tf.float32, shape=(None, None, None))
                    self.object_imagesObjectNumAll = tf.placeholder(tf.int32, shape=(None,))
                if "scene" in config.imageEnsembleChoices:
                    self.scene_imagesAll = tf.placeholder(tf.float32, shape=(None, None, None))
                    self.scene_imagesObjectNumAll = tf.placeholder(tf.int32, shape=(None,))

            elif config.imageObjects or config.imageObjectsAndGrid:
                self.imagesAll = tf.placeholder(tf.float32, shape=(None, None, None))
                self.imageBBoxesAll = tf.placeholder(tf.float32, shape=(None, None, None))
            ...
            if config.imageEnsembleFeatures:
                self.imageInDim = {}
                self.H, self.W, self.imageInDim['spatial'] = 7, 7, 2048  # config.imageDims
                self.imageInDim['object'] = 2048
                self.imageInDim['scene'] = 900
                # self.imageInDim['scene'] = 600
            else:
                self.imageInDim = config.imageDims[-1]
                if not config.imageObjects and not config.imageObjectsAndGrid and not config.imageSceneGraph:
                    self.H, self.W, self.imageInDim = 7, 7, 2048  # config.imageDims
                    if config.dataset == "CLEVR":
                        self.H, self.W, self.imageInDim = 14, 14, 1024
                if config.imageSceneGraph:
                    self.imageInDim = 900
    ...
    def createFeedDict(self, data, images, train):
        ...
        if config.imageEnsembleFeatures:
            if "spatial" in config.imageEnsembleChoices:
                feedDict.update({
                    self.imagesPlaceholder: images["spatial"]["images"],
                })
            if "object" in config.imageEnsembleChoices:
                feedDict.update({
                    self.object_imagesAll: images["object"]["images"],
                    self.object_imageBBoxesAll: images["object"]["bboxes"],
                    self.object_imagesObjectNumAll: data["objectsNums"],
                })
            if "scene" in config.imageEnsembleChoices:
                feedDict.update({
                    self.scene_imagesAll: images["scene"]["images"],
                    self.scene_imagesObjectNumAll: data["sceneObjectsNums"],
                })
        elif config.imageObjects or config.imageObjectsAndGrid:
            # print("data.objectsNums", data["objectsNums"])
            feedDict.update({
                self.imagesAll: images["images"],
                self.imageBBoxesAll: images["bboxes"],
    ...
    def initTowerBatch(self, towerI, towersNum, dataSize):
        ...
        if config.imageEnsembleFeatures:
            self.images = {}
            if "spatial" in config.imageEnsembleChoices:
                self.images['spatial'] = self.spatial_imagesAll[start:end]
            if "object" in config.imageEnsembleChoices:
                self.images['object'] = self.object_imagesAll[start:end]
            if "scene" in config.imageEnsembleChoices:
                self.images['scene'] = self.scene_imagesAll[start:end]
        else:
        ...
        if config.imageEnsembleFeatures:
            if "object" in config.imageEnsembleChoices:
                self.imageBBoxes = self.object_imageBBoxesAll[start:end]
                self.object_imagesObjectNum = self.object_imagesObjectNumAll[start:end]
            if "scene" in config.imageEnsembleChoices:
                self.scene_imagesObjectNum = self.scene_imagesObjectNumAll[start:end]

        elif config.imageObjects or config.imageObjectsAndGrid:
            self.imageBBoxes = self.imageBBoxesAll[start:end]
    ...
    def stem(self, images, inDim, outDim, addLoc=None, bboxes=None):
        with tf.variable_scope("stem"):
            if config.stemNormalize and not config.imageEnsembleFeatures:
            ...
            if config.imageObjects:  # VQA ??? or config.useBaseline:
                ...
                print("##### features", features.shape, "##### bboxes", bboxes.shape)
                features = tf.concat([features, bboxes], axis=-1)
                # features = ops.linear(features, inDim + 4, inDim, dropout=self.dropouts["stem"])
                dim = inDim + 4
                # bboxes = ops.linear(bboxes, inDim+4, dim, addBias=True, name="bbox")
                # features = features + bboxes
    ...
    def stemMultiFeatures(self, images, inDim, outDim, addLoc=None, bboxes=None):
        features = {}
        with tf.variable_scope("stem"):
            if "object" in config.imageEnsembleChoices:
                #####object
                features['object'], dim_object = images["object"], inDim['object']

                print("#####features", features['object'].shape, "#####bboxes", bboxes.shape)
                features['object'] = tf.concat([features['object'], bboxes], axis=-1)
                # features = ops.linear(features, inDim + 4, inDim, dropout=self.dropouts["stem"])
                outDim['object'] = inDim['object'] + 4

            if "scene" in config.imageEnsembleChoices:
                #####Scene
                features['scene'], outDim['scene'] = images["scene"], inDim['scene']
                if config.stemLinear:
                    features['scene'] = ops.linear(features['scene'], inDim['scene'], outDim['scene'], dropout=self.dropouts["stem"])
                if config.stemAct != "NON":
                    features['scene'] = ops.actF(config.stemAct)(features['scene'])

            if "spatial" in config.imageEnsembleChoices:
                #####Spatial
                if config.stemLinear:
                    features['spatial'] = ops.linear(images["spatial"], inDim['spatial'], outDim['spatial'])
                else:
                    if config.stemNumLayers == 0:
                        outDim['spatial'] = inDim['spatial']
                    else:
                        dims = [inDim['spatial']] + ([config.stemDim] * (config.stemNumLayers - 1)) + [outDim['spatial']]

                        images["spatial"], inDim['spatial'] = ops.addLocation(images["spatial"], inDim['spatial'], config.locationDim,
                                                                              h=self.H, w=self.W, locType=config.locationType)
                        dims[0] = inDim['spatial']

                        features['spatial'] = ops.CNNLayer(images["spatial"], dims,
                                                           batchNorm=self.batchNorm if config.stemBN else None,
                                                           dropout=self.dropouts["stem"],
                                                           kernelSizes=config.stemKernelSizes,
                                                           strides=config.stemStrideSizes)
                        if config.stemGridRnn:
                            features['spatial'] = ops.multigridRNNLayer(features['spatial'], self.H, self.W, outDim['spatial'])

                        features['spatial'] = tf.reshape(features['spatial'], (self.batchSize, -1, outDim['spatial']))
    ...
    def EnsembleMACnetwork(self, images, vecQuestions, questionWords, questionCntxWords,
                           questionLengths, name="", reuse=None, imagesDim=None):
        ...
        with tf.variable_scope("MACnetwork" + name, reuse=reuse):
            ...
            '''
            if "spatial" in config.imageEnsembleChoices:
                self.macCell['spatial'] = MACCell(
                    vecQuestions=vecQuestions,
                    questionWords=questionWords,
                    questionCntxWords=questionCntxWords,
                    questionLengths=questionLengths,
                    knowledgeBase=images['spatial'],
                    kbSize=self.H * self.W,
                    memoryDropout=self.dropouts["memory"],
                    readDropout=self.dropouts["read"],
                    writeDropout=self.dropouts["write"],
                    controlDropoutPre=self.dropouts["controlPre"],
                    ...
                    isBlock=True,
                    isTransformer=False,
                    kbDim=imagesDim['spatial'],
                    scopeName="spatial",
                    seqlen=49)

            if "object" in config.imageEnsembleChoices:
                self.macCell['object'] = MACCell(
                    vecQuestions=vecQuestions,
                    questionWords=questionWords,
                    questionCntxWords=questionCntxWords,
                    questionLengths=questionLengths,
                    knowledgeBase=images['object'],
                    kbSize=self.object_imagesObjectNum,
                    memoryDropout=self.dropouts["memory"],
                    readDropout=self.dropouts["read"],
                    writeDropout=self.dropouts["write"],
                    controlDropoutPre=self.dropouts["controlPre"],
                    ...
                    isBlock=True,
                    isTransformer=False,
                    kbDim=imagesDim['object'],
                    scopeName="object",
                    seqlen=100)

            if "scene" in config.imageEnsembleChoices:
                self.macCell['scene'] = MACCell(
                    vecQuestions=vecQuestions,
                    questionWords=questionWords,
                    questionCntxWords=questionCntxWords,
                    questionLengths=questionLengths,
                    knowledgeBase=images['scene'],
                    kbSize=self.scene_imagesObjectNum,
                    memoryDropout=self.dropouts["memory"],
                    readDropout=self.dropouts["read"],
                    writeDropout=self.dropouts["write"],
                    controlDropoutPre=self.dropouts["controlPre"],
                    ...
                    wordDropout=self.dropouts["word"],
                    vocabDropout=self.dropouts["vocab"],
                    objectDropout=self.dropouts["object"], # use the same dropout rate as obejct branch
                    batchSize=self.batchSize,
                    train=self.train,
                    reuse=reuse,
                    kbDim=imagesDim['scene'],
                    scopeName="scene",
                    seqlen=150)

            state = {}
            if "spatial" in config.imageEnsembleChoices:
                with tf.variable_scope("spatial", reuse=reuse):
                    state['spatial'] = self.macCell['spatial'].zero_state(self.batchSize, tf.float32)
            if "object" in config.imageEnsembleChoices:
                with tf.variable_scope("object", reuse=reuse):
                    state['object'] = self.macCell['object'].zero_state(self.batchSize, tf.float32)
            if "scene" in config.imageEnsembleChoices:
                with tf.variable_scope("scene", reuse=reuse):
                    state['scene'] = self.macCell['scene'].zero_state(self.batchSize, tf.float32)

            none = tf.zeros((self.batchSize, 1), dtype=tf.float32)

            for i in range(config.netLength):
                if "spatial" in config.imageEnsembleChoices:
                    self.macCell['spatial'].iteration = i
                    _, state['spatial'] = self.macCell['spatial'](none, state['spatial'])
                if "object" in config.imageEnsembleChoices:
                    self.macCell['object'].iteration = i
                    _, state['object'] = self.macCell['object'](none, state['object'])
                if "scene" in config.imageEnsembleChoices:
                    self.macCell['scene'].iteration = i
                    _, state['scene'] = self.macCell['scene'](none, state['scene'])

            finalControl, finalMemory = {}, {}
            if "spatial" in config.imageEnsembleChoices:
                finalControl['spatial'], finalMemory['spatial'] = state['spatial'].control, state['spatial'].memory
            if "object" in config.imageEnsembleChoices:
                finalControl['object'], finalMemory['object'] = state['object'].control, state['object'].memory
            if "scene" in config.imageEnsembleChoices:
                finalControl['scene'], finalMemory['scene'] = state['scene'].control, state['scene'].memory
    ...
    def outputOp(self, memory, control, vecQuestions, images, imageInDim, name=""):
        with tf.variable_scope("outputUnit"):
            ...
            if config.outQuestion:
                q = vecQuestions
                eQ = ops.linear(q, config.ctrlDim, config.memDim, name=name + "_" + "outQuestion")
            ...
            if config.outImage:
                ...
                images = ops.linear(images, config.memDim, config.outImageDim, name=name + "_" + "outImage")
    ...
    def classifier(self, features, inDim, choices=None, choicesNums=None, name=""):
        with tf.variable_scope(name + "classifier"):
    ...
    def addTopPredOp(self, logits):  # , answerFreqs
        with tf.variable_scope("pred"):
            if config.ansFormat == "oe":  # and config.ansAddUnk:
                mask = tf.to_float(tf.sequence_mask([2], config.answerWordsNum)) * (-1e30)  # 1 or 2?
                logits += mask
    ...
    def averageAcrossTowers(self, gpusNum):
        if gpusNum == 1:
            ...
            self.topkpredsAll = self.topkpredsList[0]
            ...
            self.topkpredsAll = tf.concat(self.topkpredsList, axis=0)
    ...
    def runBatch(self, sess, data, images, train, getPreds=False, getAtt=False, allData=None):
        ...
        predsOp = (self.predsAll, self.correctNumAll, self.answerAccAll, self.topkpredsAll)

        attOp = self.macCell['object'].attentions if not config.useBaseline else (self.attentions if config.baselineNew else self.noOp)
            ...
            for i in range(config.gpusNum):
                with tf.device("/gpu:{}".format(i)):
                    with tf.name_scope("tower{}".format(i)) as scope:
                        ...
                        if config.imageEnsembleFeatures:
                            outDim = {}
                            outDim['spatial'] = config.memDim
                            outDim['object'] = config.memDim
                            outDim['scene'] = config.memDim
                            imageFeatures, imageDim = self.stemMultiFeatures(self.images, self.imageInDim, outDim, bboxes=self.imageBBoxes)
                            print("############# imageDim", imageDim, "imageFeatures", imageFeatures.keys())
                            # print("#############spatial", imageFeatures['spatial'].shape)
                        elif config.imageObjectsAndGrid or config.imageObjects:
                            imageFeatures, imageDim = self.stem(self.images, self.imageInDim, config.memDim, bboxes=self.imageBBoxes)
                        ...
                        # MAC model
                            ...
                            if config.imageEnsembleFeatures:
                                finalControl, finalMemory = self.EnsembleMACnetwork(imageFeatures, vecQuestions,
                                                                                    questionWords, questionCntxWords, self.questionLengths, imagesDim=imageDim)

                                # finalFeatures = tf.concat([finalMemory['spatial'], finalMemory['object'], finalMemory['scene']], -1)
                                features_to_concat = []

                                if "spatial" in config.imageEnsembleChoices:
                                    features_to_concat.append(finalMemory['spatial'])
                                if "object" in config.imageEnsembleChoices:
                                    features_to_concat.append(finalMemory['object'])
                                if "scene" in config.imageEnsembleChoices:
                                    features_to_concat.append(finalMemory['scene'])

                                finalFeatures = tf.concat(features_to_concat, -1)

                                output, dim = self.outputOp(finalFeatures, finalControl, vecQuestions,
                                                            self.images, self.imageInDim)
                                dim = config.memDim * (len(config.imageEnsembleChoices) + 1)
                                config.outClassifierDims = [dim]
                                print("###### output shape:", output.shape, dim)
                                logits = self.classifier(output, dim, choices, self.choicesNums)
...
if __name__ == '__main__':
    parseArgs()
    config.imageObjectsAndGrid = True
    config.dataset = "GQA"
    config.ansFormat = "oe"
