...
class Preprocesser(object):
    ...
    def createVocabs(self):
        ...
        self.sceneDict.createVocab(minCount=0)

    def loadVocabs(self):
        if os.path.exists(config.qaDictFile()) and os.path.exists(config.sceneDictFile()):
    ...
    def readGQA(self, datasetFilenames, instancesFilename, sceneGraphsFilename, tier, updateVocab, imageIndex=None):
        instances = []
        ...
        if os.path.exists(instancesFilename):
            ...
            if sceneGraphsFilename is not None:
                if tier == "val" or tier == "train":
                    with open(sceneGraphsFilename, "r") as sceneGraphsFile:
                        sceneData = json.load(sceneGraphsFile)

                    for scene in tqdm(sceneData.values(), desc="Processing Scene Graphs"):
                    ...
                    # TODO Handle generation of scene graphs
                    print("Testing with scene graphs is not currently supported")
            ...
            for i in tqdm(range(len(data)), desc="Preprocessing Questions"):
                instance = data[i]

                # Remove fields that are not used
                if instance["semanticStr"] is None:
                    del instance["semanticStr"]
                if instance["semantic"] is None:
                    del instance["semantic"]
                ...
                instances.append({
                    ...
                    "tier": tier,
                    "index": i,  # The index of the question
                ...
                if config.imageObjects:
                    ...
                    instances[-1]["objectsNum"] = imageInfo["objectsNum"]
                elif config.imageEnsembleFeatures:
                    # TODO Make this part less confusing
                    instances[-1]["sceneObjectsNum"] = len(sceneData[imageId]["objects"]) if len(sceneData) != 0 else imageInfo["objectsNum"]
                    instances[-1]["objectsNum"] = imageInfo["objectsNum"]
                elif config.imageSceneGraph:
                    instances[-1]["objectsNum"] = len(sceneData[imageId]["objects"]) if len(sceneData) != 0 else imageInfo["objectsNum"]
    ...
    def readData(self, datasetFilenames, instancesFilename, sceneGraphsFilename, tier, updateVocab, imageIndex):
        # data extraction
        datasetReader = {
            "GQA": self.readGQA
        }

        return datasetReader[config.dataset](datasetFilenames, instancesFilename, sceneGraphsFilename, tier, updateVocab, imageIndex)
    ...
    def readTier(self, tier, train):
        ...
        instances = self.readData(datasetFilenames, instancesFilename, sceneGraphsFilename, tier, train, imageIndex)  # updateVocab =

        images = {tier: {"imagesFilename": imagesFilename, "imgsInfoFilename": imgsInfoFilename, "imgsSceneGraph": sceneGraphsFilename}}
    ...
    def vectorizeData(self, data):
        ...
        if config.imageObjects or config.imageObjectsAndGrid or config.imageSceneGraph:
            vectorizedData["objectsNums"] = [d["objectsNum"] for d in data]

        if config.imageEnsembleFeatures:
            vectorizedData["sceneObjectsNums"] = [d["sceneObjectsNum"] for d in data]
            vectorizedData["objectsNums"] = [d["objectsNum"] for d in data]
    ...
    def initializeWordEmbeddings(self, dim, wordsDict=None, random=False, name=""):
        ...
        if config.npy and os.path.exists(embsFile):
            ...
            print("qd", len(self.qaDict.sym2id))
            # print(self.qaDict.sym2id)
            print("scene", len(self.sceneDict.sym2id), len(embeddings))
        ...
        if wordsDict.padding in wordsDict.sym2id and wordsDict != self.answerDict and wordsDict != self.sceneDict:  # ?? padding for answers?
    ...
    def initializeQAEmbeddings(self):
        sceneEmbeddings = self.initializeWordEmbeddings(config.wrdQEmbDim, self.sceneDict,
                                                        random=config.wrdEmbQRandom, name="scene")
        ...
        if config.ansEmbMod == "SHARED":
            ...
            embeddings = {"qa": qaEmbeddings, "oeAnswers": oeAnswers, "scene": sceneEmbeddings}
        # use different embeddings for questions and answers
            ...
            embeddings = {"q": qEmbeddings, "a": aEmbeddings, "scene": sceneEmbeddings}  # mask
    ...
    def preprocessData(self, debug=False):
        ...
        return data, embeddings, self.answerDict, self.questionDict, self.sceneDict
