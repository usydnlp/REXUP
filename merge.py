...
# Merge hdf5 files
lengths = [0]
with h5py.File("data/gqa/gqa_{name}.h5".format(name = args.name)) as out:
	datasets  = {}
	...
	for i in tqdm(range(args.chunksNum)):
		with h5py.File("data/gqa/{name}/gqa_{name}_{index}.h5".format(name = args.name, index = i)) as chunk: 
...
# Update info file
with open("data/gqa/{name}/gqa_{name}_info.json".format(name = args.name)) as infoIn:
	...
	with open("data/gqa/gqa_{name}_merged_info.json".format(name = args.name), "w") as infoOut:
