...
def runEvaluation(sess, model, data, dataOps, epoch, evalTrain=True, evalTest=False,
                  getPreds=False, getAtt=None, prevRes=None, sceneDict=None, sceneEmbedding=None):
    if getAtt is None:
    ...
    if data is not None:
        if evalTrain and config.evalTrain:
            res["train"] = runEpoch(sess, model, data["evalTrain"], dataOps, train=False, epoch=epoch, getPreds=getPreds, getAtt=getAtt,
                                    ...
                                    sceneDict=sceneDict, sceneEmbedding=sceneEmbedding)
...
# Opens image files.
def openTwoImageFiles(images):
    for group in images:
        images[group]["imagesFile"] = h5py.File(images[group]["imagesFilename"], "r")
        images[group]["imagesFile_2"] = h5py.File("./data/gqa/gqa_spatial.h5", "r")
        print(group, images[group]["imagesFilename"])
        if config.dataset != "CLEVR":
            with open(images[group]["imgsInfoFilename"], "r") as file:
                images[group]["imgsInfo"] = json.load(file)
            # Closes image files.
...
# Opens image files.
def openAllImageFiles(images):
    for group in images:
        images[group]["imagesFile_object"] = h5py.File("./data/gqa/gqa_objects.h5", "r")
        images[group]["imagesFile_spatial"] = h5py.File("./data/gqa/gqa_spatial.h5", "r")
        ...
        print("########", group, images[group]["imgsSceneGraph"])
        with open(images[group]["imgsSceneGraph"], "r") as file:
            images[group]["sceneGraphFile"] = json.load(file)


# Opens SceneGraph files.
def openSceneGraphImageFiles(images):
    for group in images:
        print(group)
        with open(images[group]["imgsSceneGraph"], "r") as file:
            images[group]["sceneGraphFile"] = json.load(file)
...
def closeAllImageFiles(images):
    for group in images:
        images[group]["imagesFile_object"].close()
        images[group]["imagesFile_spatial"].close()


# Loads an images from file for a given data batch.
def loadSpatialImageBatch(images, batch):
    imagesGroup = lambda imageId: images[imageId["group"]]
    toFile = lambda imageId: imagesGroup(imageId)["imagesFile_spatial"]
    imageBatch = np.stack([toFile(imageId)["features"][imageId["idx"]] for imageId in batch["imageIds"]], axis=0)
    # config.imageDims = imageBatch.shape[1:]
    ret = {"images": imageBatch, "imageIds": batch["imageIds"]}
    return ret


def loadImageSceneGraph(images, batch, sceneDict, embedding):
    imagesGroup = lambda imageId: images[imageId["group"]]
    toFile = lambda imageId: imagesGroup(imageId)["sceneGraphFile"]
    # max_len_object 126
    # max_len_words 3480
    imageBatch = np.zeros((len(batch["imageIds"]), 150, 900))
    # imageBatch = np.zeros((len(batch["imageIds"]), 150, 600))
    for i, imageId in enumerate(batch["imageIds"]):
        # print(toFile(imageId)[imageId["idx"]].keys())
        sceneObjs = [obj["name"] for obj in toFile(imageId)[imageId["id"]]["objects"].values()]
        sceneAttrs = [obj["attributes"] for obj in toFile(imageId)[imageId["id"]]["objects"].values()]
        sceneRels = []
        for obj in toFile(imageId)[imageId["id"]]["objects"].values():
            relations = []
            for rel in obj["relations"]:
                relations.append((rel["name"], toFile(imageId)[imageId["id"]]["objects"][rel["object"]]["name"]))
...
def loadObjectImageBatch(images, batch):
    imagesGroup = lambda imageId: images[imageId["group"]]
    toFile = lambda imageId: imagesGroup(imageId)["imagesFile_object"]
    toInfo = lambda imageId: imagesGroup(imageId)["imgsInfo"][str(imageId["id"])]

    imageDim = 100
    imageBatch = np.zeros((len(batch["imageIds"]), imageDim, config.imageDims[1]))
    bboxes_Batch = np.zeros((len(batch["imageIds"]), imageDim, 4))

    for i, imageId in enumerate(batch["imageIds"]):
        numObjects = toInfo(imageId)["objectsNum"]
        imageBatch[i, 0:numObjects] = toFile(imageId)["features"][imageId["idx"], 0:numObjects]
        bboxes = toFile(imageId)["bboxes"][imageId["idx"], 0:numObjects]
        w = toInfo(imageId)["width"]
        h = toInfo(imageId)["height"]
        bboxes[:, 0] = bboxes[:, 0] / w
        bboxes[:, 1] = bboxes[:, 1] / h
        bboxes[:, 2] = bboxes[:, 2] / w
        bboxes[:, 3] = bboxes[:, 3] / h
        bboxes_Batch[i, 0:numObjects] = bboxes

    ret = {"images": imageBatch, "imageIds": batch["imageIds"], "bboxes": bboxes_Batch}
...
def loadImageBatch(images, batch):
    ...
    if config.imageObjects:
        ...
        bboxes_Batch = np.zeros((len(batch["imageIds"]), 149, 4))
        for i, imageId in enumerate(batch["imageIds"]):
            numObjects = toInfo(imageId)["objectsNum"]
        ...
        toFile_2 = lambda imageId: imagesGroup(imageId)["imagesFile_2"]
        imageDim = 100
        imageBatch = np.zeros((len(batch["imageIds"]), imageDim, config.imageDims[1]))
        bboxes_Batch = np.zeros((len(batch["imageIds"]), imageDim, 4))
...
def runEpoch(sess, model, data, dataOps, train, epoch, saver=None, calle=None,
             alterData=None, getPreds=False, getAtt=False, maxAcc=0.0, minLoss=float("Inf"), sceneDict=None, sceneEmbedding=None):
    ...
    # open image files
    openSceneGraphImageFiles(data["images"])
    if config.imageEnsembleFeatures:
        openAllImageFiles(data["images"])
    elif config.imageObjectsAndGrid:
        openTwoImageFiles(data["images"])
    else:
        ...
        try:
            ...
            if config.imageEnsembleFeatures:
                ...
                if "spatial" in config.imageEnsembleChoices:
                    imagesBatch_spatial = loadSpatialImageBatch(data["images"], batch)
                    imagesBatch["spatial"] = imagesBatch_spatial
                if "object" in config.imageEnsembleChoices:
                    imagesBatch_object = loadObjectImageBatch(data["images"], batch)
                    imagesBatch["object"] = imagesBatch_object
                if "scene" in config.imageEnsembleChoices:
                    imagesBatch_scene = loadImageSceneGraph(data["images"], batch, sceneDict, embedding=sceneEmbedding)
                    imagesBatch["scene"] = imagesBatch_scene

            elif config.imageSceneGraph:
                imagesBatch = loadImageSceneGraph(data["images"], batch, sceneDict, embedding=sceneEmbedding)
            else:
                imagesBatch = loadImageBatch(data["images"], batch)

            if not config.imageEnsembleFeatures:
                for i, imageId in enumerate(batch["imageIds"]):
    ...
    if config.imageEnsembleFeatures:
        closeAllImageFiles(data["images"])
    else:
...
def main():
    ...
    data, embeddings, answerDict, questionDict, sceneDict = preprocessor.preprocessData()
    ...
    with tf.Session(config=sessionConfig) as sess:
        ...
        if config.tensorboardGraph:
            logdir = os.path.join(config.tensorboardDir, datetime.now().strftime("%Y_%m_%d_%H_%M_%S"))
            writer = tf.summary.FileWriter(os.path.join(logdir, 'graphs'), sess.graph)
        ...
        if config.train:
            ...
            for epoch in range(config.restoreEpoch + 1, config.epochs + 1):
                ...
                trainRes = runEpoch(sess, model, trainingData, dataOps, train=True, epoch=epoch,
                                    ...
                                    sceneDict=sceneDict, sceneEmbedding=embeddings["scene"])
        ...
        if config.interactive:
            ...
            while True:
                ...
                if config.imageObjectsAndGrid:
                    instance["objectsNum"] = imageInfo["objectsNum"] + 49
